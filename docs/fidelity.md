...
| PVSG scale and initialization trace | Required research diagnostic | Every overfit and full run evaluates a fixed addressed batch at initialization, fixed early steps, and the final checkpoint. `scale_trace.jsonl` records per-window input and `q` scale, sigmoid quantiles and saturation, effective neutral score offsets versus centered data-dependent scores, index norms by candidate group, P-SA and winner feedback ratios, attention entropy, and parameter-group gradient norms. The run saves the exact normalization, initialization, gates, diagnostic addresses, cadence, and aggregation definitions. Future normalization or initialization changes receive new condition names and ledger rows and reuse the same raw features and diagnostic batch, allowing these traces to become a standalone empirical analysis rather than transient debugging output. |
| PVSG fixed-pair overfit gate | Research diagnostic | Before full training, one deterministic 200-row prefix of the immutable video-major training-pair manifest is loaded into memory and optimized as a single batch. This selection minimizes feature I/O and is never treated as an evaluation sample. Identity targets are explicit candidate positions, predicates are multi-hot train-supported targets with summed Bernoulli NLL, and each enabled unary category group contributes an unweighted cross-entropy for both entities; unavailable reviewed paths mask only that semantic term. Success requires every available target to be correct and total loss at most `0.01`. Integral training uses differentiable P-SA, while P-Samp is an evaluation-only readout of the same checkpoint. The exact rows, vocabulary, objective components, predictions, optimizer state, and scale trace are saved. |
