...
| PVSG relation-span convention | Suspected discrepancy | OpenPVSG's relation code iterates `range(start, end)`, but many annotations end at `num_frames - 1`, including apparently persistent relations. Feature extraction does not interpret spans. Before relation examples are implemented, decide and test whether the JSON endpoints are intended as half-open or inclusive; do not silently add one, clip, or discard invalid spans. |
