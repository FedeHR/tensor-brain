...
| Kaiming initialization of `V`, `B`, and `W` | Paper-reported choice | Retained for the reference implementations. Because these equations use sigmoids rather than ReLUs, changing this choice would require explicit review. |
...
| DINO features are supplied to the model | Deliberate modernization | Feature extraction remains outside the Tensor Brain core. Initial experiments add compatible features directly to `q`. |
...
| Sigmoid in the reference experiment path | Paper-faithful | `gamma = sigmoid(q)` remains the default even though the original implementation appendix reports LeakyReLU. Alternative activations may be added only as clearly named variants or knobs. |
