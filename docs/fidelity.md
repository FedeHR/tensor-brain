...
| PVSG first unary baseline comparison | Controlled thesis experiment | The selected Tensor Brain condition is QTB evolution with `softplus-bias`, Adam at `1e-3`, hierarchy supervision, shuffled 1,024-observation video chunks, and seed 0. It is compared with a local frozen-DINO linear probe, a linear head over concatenated scene and object evidence, P-Direct, a separately trained Integral QTB schedule with identity feedback disabled, and Integral P-SA. P-Samp and sequential hierarchy rollouts reuse the P-SA checkpoint. The seed-0 run is the first pass; a final stochastic-comparison claim requires replication rather than treating one favorable initialization as conclusive. |
