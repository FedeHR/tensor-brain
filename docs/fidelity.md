...
| PVSG initial feature normalization and access | Experimental preprocessing plus implementation boundary | Cached DINO tensors remain raw. For a feature dimension `D`, the initial experiment converts each addressed scene, object, and union vector to float32 and applies `sqrt(D) * L2-normalize(x)` with epsilon `1e-12`. Thus every nonzero input has component RMS one: it preserves DINO direction and removes source-dependent norm differences without pinning `sigmoid(q)` near `0.5`. Only addressed rows are normalized. Training shuffles videos and rows within each video while keeping a video's accesses contiguous, so the small raw-artifact cache does not repeatedly load and normalize whole video tables. Raw input and learned input mappings remain later ablations. |
