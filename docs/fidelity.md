...
| PVSG first pair baseline comparison | Controlled thesis experiment | Positive-pair predicate recognition trains on the complete-evidence `heldout_video/train_pairs` view and selects checkpoints on supported predicates in development videos. The selected TB condition is QTB evolution with `softplus-bias`, Adam at `1e-3`, and shuffled 1,024-observation video chunks. The objective is the paper's factorization into predicate categorical cross-entropy plus subject and object identity cross-entropies; unary hierarchy losses remain in the separate unary task. Controls are predicate frequency and directed category-pair priors, local DINO linear decoding, full-evidence fused linear and MLP heads, P-Direct, Integral without identity feedback, and Integral P-SA/P-Samp. Official evaluation videos remain untouched. |
